...
- The `kubectl` wrapper reads document `Kubernetes kubeconfig` from the 1Password `Founder` vault into a mode-`0600` temporary file for each command, then removes it. Set `KUBECTL_OP_VAULT` or `KUBECTL_OP_DOCUMENT` to override the defaults.
